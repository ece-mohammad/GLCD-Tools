...
```text
usage: image_converter.py [-h] [-o OUT_FILE] [-g] [-t THRESHOLD] [-b] [-s]
                          [-m MAX_DIM] [-z SIZE SIZE] [-p PADDING]
                          [-x COMPRESS] [-f FORMAT] [-n]
                          in_img
                          
Convert a given image (JPG/JPEG/PNG) into a bitmap/greyscale image, and/or extract its bitmap equivalent bytes into a file
